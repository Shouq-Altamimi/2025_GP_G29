...
export default function PrescriptionsPage() {
  ...
  const patientDocId  = state?.patientDocId  || fallback?.docId  || "";
  const patientName   = state?.patientName   || fallback?.name   || "";

  const [list, setList] = useState(state?.localList || []);
  ...
  const filtered = useMemo(() => {
    return list.filter(r => {
      ...
      const med = (r.medicine || r.medicineName || "").toLowerCase();
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="text-lg font-semibold text-gray-800">
                    {rx.medicine || rx.medicineName || "—"}
                  </div>
                  <div className="text-sm text-gray-600 mt-1">
                    {rx.dose || rx.dosage || "—"} • {rx.timesPerDay || rx.frequency || "—"} • {rx.durationDays || rx.duration || "—"}
                  </div>
                ...
                <div className="text-right">
                  <div className="text-xs text-gray-500">
                    {rx.createdAtTS ? rx.createdAtTS.toLocaleString() : rx.createdAt || "—"}
                  </div>
