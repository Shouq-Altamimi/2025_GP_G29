...
export default function PharmacyHistory() {
  ...
  const filtered = useMemo(() => {
    const base = rows || [];
    if (filter === "sensitive") return base.filter((r) => isSensitive(r));
    if (filter === "nonsensitive") return base.filter((r) => !isSensitive(r));
    return base;
  }, [rows, filter]);
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border bg-white p-6 text-sm" style={{ borderColor: C.line, color: C.gray }}>
            ...
            <div className="mt-2 text-xs">
              تأكدي إن الوصفات اللي تبينها فيها <b>dispensed = true</b> أو <b>dispensedAt</b>.
            </div>
          </div>
        ) : (
          filtered.map((rx) => {
